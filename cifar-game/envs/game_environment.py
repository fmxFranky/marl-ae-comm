...
def create_game_env(env_cfg, data):
    return MultiStepGame(
        env_cfg.seed,
        env_cfg.share_reward,
        data,
        env_cfg.discrete_comm,
        env_cfg.comm_size,
    )
...
class Game(gym.Env):
    ...
    def get_reward(self, env_act):
        r = [self.get_agent_reward(env_act[i], i) for i in range(self.num_agents)]
...
class MultiStepGame(Game):
    def __init__(self, seed, share_reward, data, discrete_comm, comm_size, debug=False):
        ...
        if discrete_comm:
            ...
            comm_space = spaces.Box(
                low=0.0, high=1.0, shape=(comm_size,), dtype=np.float32
            )
        self.action_space = spaces.Tuple([spaces.Discrete(11), comm_space])

        obs_space = {
            "img": spaces.Box(
                low=0, high=255, shape=self.img_shape, dtype=np.uint8  # (C, H, W)
            ),
    ...
    def reset(self):
        ...
        self.sample_idx = self.np_random.randint(low=0, high=len(self.data), size=2)
        if self.debug:
            print("Reset Multi-Step MNIST")
            print(
                "Sampled data class labels:",
    ...
    def describe(self):
        labels = [self.data[self.sample_idx[0]][1], self.data[self.sample_idx[1]][1]]
