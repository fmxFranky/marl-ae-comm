...
class FindGoalMultiGrid(MultiGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        if getattr(self, "randomize_goal", True):
            goal_pos = self.place_obj(Goal(color="green", reward=1), max_tries=100)
        else:
    ...
    def gen_global_obs(self, agent_done=None):
        ...
        self.sees_goal = (
            np.array(
                [
                    self.agents[i].in_view(self.goal_pos[0], self.goal_pos[1])
        ...
        obs = {
            ...
            "pos": np.stack(
                [self.get_agent_pos(a) for a in self.agents], axis=0
            ),  # (N, 2)
            "comm_act": np.stack(
    ...
    def reset(self):
        ...
        if self.num_adversaries < 0:
            ...
            adv_indices = np.random.choice(
                [i for i in range(self.num_agents)], self.num_adversaries, replace=False
            )
            for i, agent in enumerate(self.agents):
                if i in adv_indices:
    ...
    def update_reward(self, step_rewards):
        ...
        ndis_to_goal = [
            dis_func(agent.pos, self.goal_pos, k=self.max_dis) for agent in self.agents
        ]
    ...
    def step(self, action_dict):
        ...
        info_dict = {
            f"agent_{i}": {
                ...
                "posd": np.array(
                    [self.agents[i].pos[0], self.agents[i].pos[1], done_n[i]]
                ),
        ...
        info_dict["rew_by_act"] = {
            ...
            "comm": {f"agent_{i}": comm_rewards[i] for i in range(len(comm_rewards))},
        }

        # team reward
        if self.separate_rew_more:
            info_dict["rew_by_act"][1] = {
                f"agent_{i}": (step_rewards[i] - env_rewards[i])
                for i in range(len(step_rewards))
            }
        else:
            info_dict["rew_by_act"][1] = {
                f"agent_{i}": (step_rewards[i]) for i in range(len(step_rewards))
            }
