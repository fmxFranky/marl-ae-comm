            ...
            for eval_id in range(self.num_eval_episodes):
                ...
                while not check_done(done):
                    if self.net_type == "ae":
                        ...
                        _, _, ent, action = self.net.take_action(plogit, comm_out)
                    else:
                    ...
                    if hasattr(env_copy, "get_raw_obs"):
                        ...
                        warnings.warn(
                            "environment does not have get_raw_obs() "
                ...
                plot_ents(
                    ...
                    agent_steps,
                    ent_path,
                    env_copy.adv_indices,
                    max_ent=[np.log(5), np.log(2)],
                )

                # save video
                latest_path = osp.join(self.video_save_dir, f"latest_{eval_id}.mp4")
                make_video(latest_path, frames, fps=self.fps, verbose=False)

                # only save the first video in history
                if (weight_iter + 1) % self.video_save_freq == 0 and eval_id == 0:
                    save_path = osp.join(self.video_save_dir, f"{weight_iter}.png")
                ...
                for i, agent in enumerate(self.agents):
                    ...
                    ctime = max_time if agent_steps[i] < 0 else agent_steps[i]
