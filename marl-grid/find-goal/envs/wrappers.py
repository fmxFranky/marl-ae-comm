    ...
    def step(self, action):
        ...
        for k, v in obs_dict.items():
            ...
            if isinstance(v, dict):
                obs_dict[k]["pov"] = 2.0 * ((v["pov"] / 255.0) - 0.5)
            else:
                obs_dict[k] = 2.0 * ((v / 255.0) - 0.5)
    ...
    def render_reward(self, frame, reward_dict, info_dict):
        if self.show_reward:
            ...
            to_render = [
                ...
                *[
                    f"{k[0] + k[-1]}: {v:.3f}"
                    for k, v in info_dict["rew_by_act"][1].items()
                ],
            ]
            ...
            for k, v in info_dict.items():
                if k[-1].isdigit() and v["comm_str"] != "":
                    to_render += [(str(k[0] + k[-1]) + " ---"), *(v["comm_str"])]

            str_spacing = 30
            x_start = ((frame.shape[1] - frame.shape[0]) // 2) + frame.shape[0] + 10
